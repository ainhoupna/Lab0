...
@click.group(
    help="CLI for the MLOps assignment: Fundamentals of Continuous Integration (CI)."
)
def cli():
    """
...
@cli.group(
    help="Functions related to data structure manipulation (lists, etc.)."
)
def struct():
...
def parse_list_argument(_ctx, _param, value) -> List[Any]:
    ...
    # ctx, param renamed to _ctx, _param to satisfy W0613
    try:
...
def parse_numeric_list_argument(
    _ctx, _param, value
...
@clean.command(help="Removes missing values (None, '', nan) from a list.")
...
@numeric.command(help="Normalizes numerical values using the Min-Max method.")
...
@numeric.command(help="Clips numerical values to a minimum and maximum range.")
@click.argument("values", type=str, callback=parse_numeric_list_argument)
@click.option(
    "--min_clip",
    type=float,
...
def clip(_ctx, values, min_clip, max_clip):
    ...
    # The logic correctly uses clip_values which can return floats/ints.
...
@text.command(help="Removes specified stop-words from the text (lower-cased).")
@click.argument("input_text", type=str)
@click.option(
    "--stop_words",
    "-s",
...
def shuffle(_ctx, values, seed):
    ...
    # The logic correctly uses random_shuffle_list, which has deterministic output for seed=42
    result = random_shuffle_list(values, seed)
    click.echo(f"Result: {result}")


@struct.command(help="Flattens a list of lists into a single list.")
